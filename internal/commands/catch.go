...
func catch(args []string, cfg *Config) error {
	...
	if args[0] == "" {
		return fmt.Errorf("No pokemon name to catch passed as an argument.")
	}
	url = url + args[0]
	...
	catchRate := getCatchRate(baseExp)
...
func getCatchRate(baseExp int) float64 {
	if baseExp < 1 {
		baseExp = 1
	}
	...
	a := 1.1
	b := 0.2
	rate := a - b*math.Log(float64(baseExp))
